...
PidTypeDef_t PID_DC_AC[3];  // 逆变内环
PidTypeDef_t PID_AC_DC[3];  // 整流内环
 ...
 * Output:          NULL
---------------------------------------------------------------*/
void PID_init(PidTypeDef_t *pid, uint8_t mode, const float PID[3], float max_out, float min_out)
{
...
float PID_Calc(PidTypeDef_t *pid, float ref, float set)
{
    ...
    if (pid->mode == PID_POSITION)			//采用位移式PID
    {
        ...
        float_clamp(&pid->Iout, pid->max_out, pid->min_out);
        pid->out = pid->Pout + pid->Iout + pid->Dout;
        float_clamp(&pid->out, pid->max_out, pid->min_out);
    }
